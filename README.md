...
- **Pendiente:** seguir ampliando el catálogo propio (7 componentes hoy) — rivales conocidos ya muestran 14+, aunque muchos de los suyos son átomos genéricos que nuestro catálogo básico ya cubre; el modal HITL de "Atender" (hechos deterministas + "Ver propuesta del asistente" bajo demanda) existe en React pero no en Flutter, donde "Atender" ejecuta directo sin mostrar nada; `confirm_action` sigue devolviendo una tarjeta fija sin pasar por el modelo.
